        ...
        public:
            ...
            static ECS::BaseComponent *deserialize(const std::vector<char>& vec, ECS::BaseComponent* component = nullptr) {
                ...
                while (iss.tellg() < vec.size()) {
                    ...
                    if (component == nullptr) {
                        component = new EnemyQueueComponent(std::vector<std::pair<bool, std::pair<std::tuple<size_t, float, bool>, std::function<size_t(float, bool)>>>>({std::make_pair(hasSpawned, std::make_pair(std::make_tuple(x, y, isAttacking), std::function<size_t(float, bool)>()))}));
                    } else {
                        EnemyQueueComponent* enemyQueueComponent = dynamic_cast<EnemyQueueComponent*>(component);
                        if (enemyQueueComponent == nullptr) return nullptr;
                        enemyQueueComponent->enemyQueueFactories.push_back(std::make_pair(hasSpawned, std::make_pair(std::make_tuple(x, y, isAttacking), std::function<size_t(float, bool)>())));
                    }
                }
                return component;
            }
